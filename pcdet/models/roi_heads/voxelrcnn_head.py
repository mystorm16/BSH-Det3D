...
class VoxelRCNNHead(RoIHeadTemplate):
    def __init__(self, backbone_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        self.bev_conv = nn.Sequential(
            nn.Conv2d(256, 32, kernel_size=(1, 1), stride=(1, 1), padding=(1, 1), bias=False),
            nn.BatchNorm2d(32, eps=0.001, momentum=0.01, affine=True, track_running_stats=True),
            nn.ReLU(),
        )
        self.ADF_blocks_roi = ADF(128)
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        ADF_features_2d = self.bev_conv(batch_dict['ADF_features_2d'])
        idx = torch.arange(batch_size, device=roi_grid_xyz.device).view(-1, 1)\
            .repeat(1, roi_grid_xyz.shape[1]).view(-1, 1)
        keypoints = torch.cat((idx.float(), roi_grid_xyz.view(-1, 3)), dim=1)
        if self.model_cfg.ROI_GRID_POOL.BEV_FEATURES:
            point_bev_features = self.interpolate_from_bev_features(
                keypoints, ADF_features_2d, batch_dict['batch_size'],
        ...
        ms_pooled_features_att = ms_pooled_features.permute(0, 2, 1).unsqueeze(2)
        ms_pooled_features_att = self.ADF_blocks_roi(ms_pooled_features_att)
        ms_pooled_features_att = ms_pooled_features_att.squeeze(2).permute(0, 2, 1)

        return ms_pooled_features_att

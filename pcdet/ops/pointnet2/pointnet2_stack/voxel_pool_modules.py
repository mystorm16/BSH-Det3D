...
class NeighborVoxelSAModuleMSG(nn.Module):
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, \
        ...
        new_coords = new_coords[:, [0, 3, 2, 1]].contiguous()  # grid point的voxel坐标
        new_features_list = []
        for k in range(len(self.groupers)):
            # features_in: (1, C, M1+M2)
            features_in = features.permute(1, 0).unsqueeze(0)  # features为每个非空voxel的特征
            ...
            )  # 完成voxel query的操作，并将选取到的所有点group到一起

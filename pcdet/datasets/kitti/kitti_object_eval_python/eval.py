    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                # sy_add
                if m == 0 and l == 1 and min_overlap == 0.7 and metric == 2:
                    detail_result.append(np.hstack((np.expand_dims(thresholds, axis=1), pr[:, 0:3],
                                                    np.expand_dims(precision[0, 1, 0][0:len(pr[:, 0:3])], axis=1))))
...
def do_eval(gt_annos,
    ...
    if pr_rc_details is not None:
        pr_rc_details['bev'] = create_details(current_classes, ret["precision"], ret["real_recall"],
    ...
    detail_result = ret["detail_result"]
    if PR_detail_dict is not None:
        PR_detail_dict['3d'] = ret['precision']

    if pr_rc_details is not None:
        pr_rc_details['3d'] = create_details(current_classes, ret["precision"], ret["real_recall"],
                                             difficultys=difficultys)

    return mAP_bbox, mAP_bev, mAP_3d, mAP_aos, mAP_bbox_R40, mAP_bev_R40, mAP_3d_R40, mAP_aos_R40, detail_result


def create_details(current_classes, pc, rc, difficultys=None):
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, coverage_rates=None, PR_detail_dict=None):
    ...
    for anno in dt_annos:
        if anno['alpha'].shape[0] != 0:
            ...
            break
    pr_rc_details = {}
    mAPbbox, mAPbev, mAP3d, mAPaos, mAPbbox_R40, mAPbev_R40, mAP3d_R40, mAPaos_R40, detail_result = do_eval(
        gt_annos, dt_annos, current_classes, min_overlaps, compute_aos, PR_detail_dict=PR_detail_dict,
        pr_rc_details=pr_rc_details, coverage_rates=coverage_rates)
    ...
    return result, ret_dict, pr_rc_details, detail_result

...
class SupGTSampling(object):
    ...
    def add_sampled_boxes_best_match_points_to_scene(self, data_dict, sampled_gt_boxes, boxes_oriyaw,
                                                     total_valid_sampled_dict, aug_boxes_image_idx, aug_boxes_gt_idx,
                                                     valid_boxes_type):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            if type > 0:
                ...
                bm_obj_points = np.concatenate([bm_obj_points, np.tile(avg_feature, (bm_obj_points.shape[0], 1))],
                                               axis=-1)
        ...
        if len(gt_smp_points_list) > 0:
            ...
            if self.gt_smp_cfg["remove_yz_expansion"] > 0:
                smp_points, pnt_num_mask = self.remove_occ(points, smp_points, sampled_gt_boxes[smp_inds, :],
                                                           self.gt_smp_cfg["remove_yz_expansion"],
                                                           self.gt_smp_cfg["dp_rate"])
                smp_inds = smp_inds[pnt_num_mask]
                final_bm_points_list.extend(
                    [gt_smp_bm_points_list[i] for i in range(len(gt_smp_bm_points_list)) if pnt_num_mask[i]])
        ...
        if len(bm_points_list) > 0:
            ...
            if self.mlt_bm_cfg["remove_yz_expansion"] > 0:
                bm_points, pnt_num_mask = self.remove_occ(points, bm_points, sampled_gt_boxes[bm_inds, :],
                                                          self.mlt_bm_cfg["remove_yz_expansion"],
                                                          self.mlt_bm_cfg["dp_rate"])
        ...
        if len(final_bm_points_list) > 0:
            ...
            if gt_boxes.ndim != 2 or gt_boxes.shape[0] == 0:
                ...
                gt_boxes = final_sampled_gt_boxes[0] if len(final_sampled_gt_boxes) == 1 else np.concatenate(
                    final_sampled_gt_boxes, axis=0)
            else:
            ...
            data_dict['aug_boxes_image_idx'] = final_aug_boxes_image_idx[0] if len(
                final_aug_boxes_image_idx) == 1 else np.concatenate(final_aug_boxes_image_idx, axis=0)
            data_dict['aug_boxes_gt_idx'] = final_aug_boxes_gt_idx[0] if len(
                final_aug_boxes_gt_idx) == 1 else np.concatenate(final_aug_boxes_gt_idx, axis=0)
    ...
    def expand_voxel(self, bm_occ_coords, yz_epansion=2):
        ...
        expand_bm_occ_coords = np.clip(np.expand_dims(bm_occ_coords, axis=1) + xyz_shift, a_min=0, a_max=None).reshape(
            -1, 3)
    ...
    def remove_occ(self, points, bm_points, sampled_gt_boxes, yz_epansion, drop_rate):
        if yz_epansion > 1:
            ...
            sphere_coords_min = np.min(np.concatenate([occ_coords_points, bm_occ_coords_points], axis=0), axis=0,
                                       keepdims=True)
            occ_coords = np.floor_divide(occ_coords_points - sphere_coords_min, self.sphere_coords_res).astype(np.int32)
            bm_occ_coords = np.floor_divide(bm_occ_coords_points - sphere_coords_min, self.sphere_coords_res).astype(
                np.int32)
            ...
            voxelwise_mask[occ_coords[..., 0], occ_coords[..., 1], occ_coords[..., 2]] = np.ones_like(
                occ_coords[..., 0], dtype=np.uint8)
    ...
    def add_box_jitter(self, sampled_boxes, points, existed_boxes, cfg):
        ...
        center_change = np.clip(centers[..., 0] + cfg['box_range_jitter'], a_min=range_scale[0],
                                a_max=range_scale[1]) - center_base_min
        ...
        rot_change = np.clip(centers[..., 1] + cfg['box_rot_jitter'], a_min=rot_scale[0],
                             a_max=rot_scale[1]) - rot_base_min
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                gt_num, bm_num = 0, 0
                if self.sampler_cfg.get('GT_SMP', None) is not None and self.gt_smp_cfg['sample_groups'][
                    class_name] > 0:
                    gt_num = min(self.gt_smp_cfg['sample_groups'][class_name], len(sampled_dict))
                    sampled_boxes_lst.append(
                        self.add_box_jitter(sampled_boxes[:gt_num, :], data_dict['points'], existed_boxes,
                                            self.gt_smp_cfg))
                ...
                if self.sampler_cfg.get('MLT_BM', None) is not None and bm_num > 0:
                    sampled_boxes_lst.append(
                        self.add_box_jitter(sampled_boxes[len(sampled_boxes) - bm_num:, :], data_dict['points'],
                                            existed_boxes, self.mlt_bm_cfg))
                    sampled_boxes_type_lst.append(np.zeros([bm_num]))
                sampled_boxes = sampled_boxes_lst[0] if len(sampled_boxes_lst) == 1 else np.concatenate(
                    sampled_boxes_lst, axis=0)
                sampled_boxes_type = sampled_boxes_type_lst[0] if len(sampled_boxes_type_lst) == 1 else np.concatenate(
                    sampled_boxes_type_lst, axis=0)
                ...
                if self.no_stucking:
                    det_boxes = copy.deepcopy(valid_sampled_boxes[:, 0:7])
                    det_boxes[:, 2] += self.det_height_shift
                    clean_mask = box_utils.boxes3d_contain_point_thresh(data_dict['points'], det_boxes, thresh=0,
                                                                        smaller=True).nonzero()[0]
                    valid_boxes_type = valid_boxes_type[clean_mask]
                    valid_sampled_dict = [valid_sampled_dict[x] for x in clean_mask]
                    valid_sampled_boxes = valid_sampled_boxes[clean_mask]
                    valid_sampled_boxes_image_idx = valid_sampled_boxes_image_idx[clean_mask]
                    valid_sampled_boxes_gt_idx = valid_sampled_boxes_gt_idx[clean_mask]
                valid_sampled_boxes, valid_sampled_boxes_oriyaw = valid_sampled_boxes[..., :7], valid_sampled_boxes[...,
                                                                                                7:8]
        ...
        if total_valid_sampled_dict.__len__() > 0:
            data_dict = self.add_sampled_boxes_best_match_points_to_scene(data_dict, sampled_gt_boxes,
                                                                          valid_sampled_boxes_oriyaw,
                                                                          total_valid_sampled_dict,
                                                                          valid_sampled_boxes_image_idx,
                                                                          valid_sampled_boxes_gt_idx, valid_boxes_type)
